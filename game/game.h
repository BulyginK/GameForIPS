...
void drawSidebar(sf::RenderWindow &window, int &lives, sf::Sprite playerSprite, sf::RectangleShape livesDesignationBackgr, std::vector<PrizeEffect> &prizeEffects)
{
    ...
    sf::Vector2f initPos = {x : 50, y : 370};
    livesDesignationBackgr.setPosition(43 + 57 * (lives - 1), 368);
...
void adjustSidebar()
{
    ...
    playerText.setPosition({x : 510, y : 80});
    ...
    scoresText.setString(std::to_string(scores));
    scoresText.setPosition({x : 510, y : 120});
    ...
    highScoresText.setPosition({x : 370, y : 200});
    highScoresText.setFont(getFont());
    highScoresText.setCharacterSize(30);
    highScoresText.setFillColor(sf::Color(198, 171, 66));
    highScoresText.setString("High score");

    highScore = sf::Text();
    highScore.setFillColor(sf::Color(198, 171, 66));
    highScore.setFont(getFont());
    highScore.setCharacterSize(30);
    highScore.setString(highScoresStr);
    highScore.setPosition({x : 600, y : 200});
}
...
void playGame(sf::RenderWindow &window)
{
    ...
    while (window.isOpen())
    {
        ...
        updatePrizeEffects(dt, koeffOfPlatformExpansion, koeffOfBallSpeed, ballSpeed, isPaused, prizeEffects);
